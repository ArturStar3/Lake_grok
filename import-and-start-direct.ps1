# Импорт Docker-образов и запуск БЕЗ nginx (production-статика на :5173).
param(
    [string]$TarFile = ""
)
...
if (-not $TarFile) {
    if (Test-Path "infolake_full_offline_direct.tar") {
        $TarFile = (Resolve-Path "infolake_full_offline_direct.tar").Path
    } else {
        $TarFile = Get-ChildItem -Filter "infolake_full_offline_direct_*.tar" |
...
if (-not $TarFile -or -not (Test-Path $TarFile)) {
    Write-Host "Archive not found (infolake_full_offline_direct.tar)" -ForegroundColor Red
    Write-Host "On online machine run: .\export-offline.ps1 -Direct" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "`nStarting containers (offline DIRECT, no nginx, no build, no pull)..." -ForegroundColor Cyan
...
Write-Host "  App:   http://localhost:$uiPort/"
